...
- [ ] 업데이트 존재 시 실행 2초 후 하단 status bar + 타이틀바 " — Update Available" 표시 (24h 창 내면 생략) ✅
...
- `bridge.js`는 `prepare-frontend.sh` 경로 외에서 포매터·번들러·린터에 태우지 말 것. `// @dev-hook-start` / `// @dev-hook-end` 주석이 릴리즈 빌드에서 훅을 제거하는 sentinel이므로 주석이 지워지면 테스트 표면이 프로덕션에 유출된다
- 한국어로 소통
